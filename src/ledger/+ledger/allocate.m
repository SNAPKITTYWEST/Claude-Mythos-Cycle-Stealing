...
function [ledger, eventId] = allocate(ledger, workerId, cycles)
    % Allocate cycles to a worker
    % Maintains cycle conservation: total allocated <= available
    ...
    if cycles > ledger.availableCycles
        error('Insufficient cycles: requested %d, available %d', cycles, ledger.availableCycles);
    end
    ...
    if ledger.eventCount < ledger.maxEvents
        ...
        error('Event log full (max %d events)', ledger.maxEvents);
    end
